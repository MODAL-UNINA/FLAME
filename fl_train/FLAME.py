...
# kernel density estimation for the last feature layer aggregation, other layers are aggregated by the average
def flame(w, bandwidth, weight_scalling_factor_list):
    last_layer_weight_name = list(w[0].keys())[-4]
    last_layer_bias_name = list(w[0].keys())[-3]
    ...
    # using KDE get the kernel density
    kde_w = KernelDensity(kernel='gaussian', bandwidth=bandwidth).fit(w_last_layer, sample_weight=weight_scalling_factor_list)
    kde_b = KernelDensity(kernel='gaussian', bandwidth=bandwidth).fit(b_last_layer, sample_weight=weight_scalling_factor_list)

    # sampling a new last feature layer for the global model
    ...
    # update weights except last feature layer
    ...
    for k in w_global.keys():
        if k != last_layer_weight_name and k != last_layer_bias_name:
            ...
            for i in range(len(w)):
                w_global[k] = w_global[k] + (w[i][k] * weight_scalling_factor_list[i])

    # update last feature layer
    w_global[last_layer_weight_name] = torch.tensor(w_last_layer_new.reshape(w_global[last_layer_weight_name].shape)).to(device)
    w_global[last_layer_bias_name] = torch.tensor(b_last_layer_new.reshape(w_global[last_layer_bias_name].shape)).to(device)
...
for iter in range(100):    
    ...
    w_glob = flame(w_locals, bandwidth, weight_scalling_factor_list)
