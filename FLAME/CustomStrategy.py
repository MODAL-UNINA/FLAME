...
class Dis(fl.server.strategy.FedAvg):
    ...
    def aggregate_fit( 
        ...
        # Get the model parameters, jsd_value and optimal weights for each client.
        ...
        # Calculate the average jsd_value of all clients
        ...
        for (local_parameters, best_weights, num_examples, jsd_value) in weights_results:
            if jsd_value < average_jsd_value:
                # When jsd_value is less than the average, use the current weight
                results.append((local_parameters, num_examples))
            else:
                # else, use the best weights
    ...
    def aggregate_flame(self, results: List[Tuple[NDArrays, int]]) -> NDArrays:
        ...
        kde_w = KernelDensity(kernel='gaussian', bandwidth=0.01).fit(w_last_layer, sample_weight=weight_scalling_factor_list)
        kde_b = KernelDensity(kernel='gaussian', bandwidth=0.01).fit(b_last_layer, sample_weight=weight_scalling_factor_list)
